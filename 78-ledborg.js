...
module.exports = function(RED) {
    ...
    function LedBorgNode(n) {
        ...
        if (LedBorgInUse) { this.error("LEDborg节点已在使用中，您可能只有一个。"); }
        ...
        function inputlistener(msg) {
            ...
            if (node.child !== null) {
                ...
                node.warn("命令未运行");
                node.status({fill:"red",shape:"ring",text:"命令未运行"});
            }
        }
        ...
        if (allOK === true) {
            ...
            node.child.on('error', function (err) {
                if (err.errno === "ENOENT") { node.warn('C命令未运行'); }
                else if (err.errno === "EACCES") { node.warn('命令不可执行'); }
    ...
    RED.nodes.registerType("Raspberry Pi",LedBorgNode);
}
